...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(
            msg.sender == _owner,
            "The function is not being called by the owner!"
        );
        _;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(
            newOwner != address(0),
            "Adress provided is not a real address!"
        );
...
contract Pausable is Ownable {
    ...
    event Paused(address person);
    event Unpaused(address person);
    ...
    function setPaused(bool pause) public onlyOwner {
        require(
            pause != _paused,
            "Contract is already in the state that you sent!"
        ); // I put this require so that the owner doesn't send a similar value to the one already put and not waste ether.
        _paused = pause;
        if (pause == true) {
...
contract ERC721 is Pausable, ERC165 {
    ...
    function _transferFrom(
        ...
        require(
            to != address(0),
            "Ownership of token is not being transferred to a valid address!"
        );
        // TODO: clear approval
...
contract FinalERC721Token is
    ERC721Metadata(
        "Real Estate Marketplace By Mira Bahou",
        "REMBMB",
        "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/"
    )
{
    function mint(address to, uint256 tokenId) public returns (bool) {
        //I removed "string memory tokenURI" from the parameteres because it wasn't used in this function!
        require(
            owner() == msg.sender,
            "Function is not being called by owner!"
        );
        super._mint(to, tokenId);
